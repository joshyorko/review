...
The Hive runtime inside is tracked directly from upstream's `v4` branch and is **never pinned** to a static commit in this repository. Every image build resolves the branch to a commit, stamps it into `/usr/share/hive/contribute/HIVE_COMMIT`, records it in the image labels (`io.hivecommons.contribute.hive.ref`) and SBOM, and registration clones that same branch so setup and runtime cannot drift.
