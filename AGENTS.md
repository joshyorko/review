...
Every local Podman launcher path that can execute agent-controlled work uses
the Review-owned host-side `runsc` OCI runtime explicitly. The launcher first
runs a credential-free rootless Podman probe and verifies the probe container's
...
configured default. Kubernetes, remote Podman, and broker Jobs execute on
node-owned runtimes and require separate runtime proof; local evidence never
claims their gVisor identity. Build, audit, and unrelated utility containers
are not agent execution paths and do not inherit this runtime policy.
