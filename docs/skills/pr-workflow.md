...
For an OMP integration candidate, keep repository grouping and autoslay behind
the fresh admission check on every dispatch, including turn-end continuations.
Retain live landing preflights when adopting dashboard changes. UI tests that
plan a landing must supply both PR evidence and a repository review policy;
otherwise the fixture depends on the host's GitHub state.
