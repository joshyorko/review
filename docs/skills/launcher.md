...
description: "Maintains the review recipes, host runtime boundary, and credential boundaries. Use when editing justfile."
metadata:
  type: runbook
...
1. Keep exactly seven public recipes:
   ...
   | `review-runtime` | Install, update, inspect, or remove the Review-owned host gVisor bundle. |
...
## Review-Owned Host Runtime

Agent-capable local Podman launches require the Review-owned pinned gVisor
bundle under `/var/lib/bluefin-review/runtime/gvisor/<release>/`. Install it
...
It never falls back to `crun`, `runc`, or Podman's default runtime. Kubernetes,
remote Podman, and broker Jobs have separate node-owned runtime proof; local
runsc evidence does not establish their gVisor identity.
...
The recipe list must contain only the seven public commands. Doctor must not start an agent or persist a runtime bundle.
