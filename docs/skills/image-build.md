...
`image/contribute/Containerfile` is the separate distroless Hive + OMP closure. It stages only OMP, Node, GitHub CLI, tmux, locked `ws`, the merged Hive runtime, and the FSDK shell/git closure; it must not absorb Codex, Pi, Python, dashboard, review scope, or generated skills.
