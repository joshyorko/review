...
With the architecture shift to a single consolidated check prompt per review
pass (`docs/skills/review-checks.md`), host review slots no longer spawn
multiple child subagent process trees. Aligning with
`MAX_CONCURRENT_LANDINGS = 7`, `DEFAULT_REVIEW_CAP = 7` provides 7 concurrent
review slots process-wide while respecting machine bounds (`cores // 2` and
memory headroom). Sized for high-throughput unattended burns, the scheduler
admits up to 7 review tasks concurrently, while phase tasks and Hive review
agents operate outside the worker cap.
