...
| **Managed Reviewer Client** | A foreground review session (OMP review mode, or retained compatibility session) that a Maintainer Reviewer runs to examine evidence and prepare reviews. | It prepares a Review Draft or executes typed, human-confirmed decisions: App-authored approval with `lgtm` opt-in, or direct squash merge (gated on GitHub `push` permission). It decides nothing itself. |
