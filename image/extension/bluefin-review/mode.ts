...
export class ReviewMode {
	...
	private excludeUnsupportedPullRequests(items: readonly QueueItem[]): QueueItem[] {
		...
		for (const item of items) {
			...
			const visibleForInspection = this.env.BLUEFIN_REVIEW_SHOW_WORKFLOW_PRS === "1";
			if (unsafeForLanding && !visibleForInspection && !isRepairRequested(item, this.currentUserLogin)) {
