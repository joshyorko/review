		...
		switch (action.kind) {
			...
			case "slay":
				...
				return batch.every((entry) => entry.type === "issue")
					? `Close out the following ${batch.length} queued issues by shipping the work, one pull request per issue:\n\n${list}\n\n${crossRepoHeader ? `${crossRepoHeader}\n\n` : ""}For each issue: do not dismiss or conclude no_work_needed if there is an actionable bug, missing test, broken script, or underlying root cause to address. Diagnose the root cause, implement the fix, run the smallest existing test covering the changed surface, and open a pull request that closes it with \`Closes <owner/repo>#<number>\` in the body. If the implementation is complete but repository-local verification tooling is unavailable in this environment (e.g. missing Python/Rust/Node/Poetry/uv runtimes), do not discard completed work: push the branch, open a draft pull request with \`gh pr create --draft\`, explicitly record which verification passed and which runtime was unavailable, and let repository CI verify it. Someone else reviews and merges: never merge your own, never approve. Only if an issue has genuinely already been merged by an earlier PR on the default branch: confirm that commit and close the issue directly with \`gh issue close <number> --repo <owner/repo> --reason completed --comment "<evidence of prior merged PR>"\`. Where an issue cannot be finished as asked, open no pull request for it and report an evidenced finding instead, naming what blocked you.\n\n${protocol}`
	...
	switch (action.kind) {
		...
		case "slay":
			...
			return action.item.type === "issue"
				? `Close out ${cite(action.item)} by implementing and shipping the solution. Do not dismiss or conclude with no_work_needed if there is any actionable bug, test failure, code change, documentation fix, or underlying root cause to address. Inspect the code, diagnose the problem, implement the fix, run the smallest existing test that covers the changed surface, then open a pull request against the default branch whose body contains \`Closes ${action.item.repo}#${action.item.id}\`. If the implementation is complete but repository-local verification tooling is unavailable in this environment (e.g. missing Python/Rust/Node/Poetry/uv runtimes), do not discard completed work: push the branch, open a draft pull request with \`gh pr create --draft\`, explicitly record which verification passed and which runtime was unavailable, and let repository CI verify it. Someone else reviews and merges it: never merge your own, never approve it. Only if the issue has already been resolved or closed by an existing merged PR or commit on the default branch: confirm the evidence and close the issue directly with \`gh issue close ${action.item.id} --repo ${action.item.repo} --reason completed --comment "<evidence of live resolution or commit>"\`. Otherwise implement what it asks for and open the PR.${hive}`
